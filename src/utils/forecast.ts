...
export function buildForecast(data: Partial<AppState>, horizonDays = 60): ForecastResult {
  ...
  for (let i = 1; i <= horizonDays; i++) {
    ...
    // Recurring incomes
    (data.recurringIncomes ?? []).filter((r) => r.isActive).forEach((inc) => {
      if (inc.nextDate?.slice(0, 10) === ds)
        events.push({ date: ds, label: inc.name, amount: inc.amount, type: "income" });
    });

    // Recurring payments
    (data.recurringPayments ?? []).filter((r) => r.isActive).forEach((rp) => {
    ...
    (data.goals ?? []).filter((g) => g.status === "active" && g.monthlyContribution).forEach((g) => {
      // Contribute on 1st of each month in horizon
      const d = addDays(today, i);
      if (d.getDate() === 1)
        events.push({ date: ds, label: `${g.name} contribution`, amount: -(g.monthlyContribution ?? 0), type: "goal_contribution" });
    });
  }
  ...
  const monthlyObligations =
    (data.recurringPayments ?? []).filter((r) => r.isActive).reduce((s, r) => s + (r.amount ?? 0), 0) +
    (data.loans ?? []).reduce((s, l) => s + (l.emi ?? 0), 0) +
    (data.goals ?? []).filter((g) => g.status === "active").reduce((s, g) => s + (g.monthlyContribution ?? 0), 0);
