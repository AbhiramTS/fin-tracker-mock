...
interface AppContextValue {
  state: AppState;
  save: (entity: EntityName, record: Partial<BaseRecord>) => Promise<BaseRecord>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const save = useCallback(async (entity: EntityName, record: Partial<BaseRecord>) => {
    const saved = await Repos[entity].save(record as Parameters<typeof Repos[typeof entity]["save"]>[0]);
  ...
  const connectFirebase = useCallback(async (config: FirebaseConfig) => {
