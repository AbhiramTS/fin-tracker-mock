...
export function registerSyncAdapter(adapter: SyncAdapter, onRealtimeUpdate?: (entity: string) => void): void {
  _adapter?.destroy();
  _adapter = adapter;
...
export async function enqueueChange(params: { entity: string; type: ChangeType; payload: Record<string, unknown> }): Promise<void> {
  await dbPut("syncQueue", { queueId: generateId(), ...params, createdAt: new Date().toISOString(), syncedAt: null } as ChangeRecord);
  flush();
}
...
export async function flush(): Promise<void> {
  ...
  } catch (e) { console.warn("[SyncQueue]", (e as Error).message); }
