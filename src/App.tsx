...
import {
  LayoutDashboard, Receipt, Wallet, TrendingUp, RefreshCw,
  ArrowLeftRight, Target, CreditCard, Landmark, Users,
  BarChart3, Sliders, Settings, Menu, X, Zap,
} from "lucide-react";
...
// ── Nav config ────────────────────────────────────────────────────────────────
type TabId =
  | "dashboard" | "expenses" | "income" | "transfers" | "recurring"
  | "loans" | "cards" | "receivables" | "investments" | "goals"
  | "forecast" | "simulator" | "accounts" | "reconciliation" | "settings";

interface NavItem { id: TabId; label: string; icon: React.ComponentType<{className?:string}>; group?: string }
...
// ── Sidebar ───────────────────────────────────────────────────────────────────
function Sidebar({ tab, setTab, onClose }: { tab: TabId; setTab: (t: TabId) => void; onClose?: () => void }) {
  ...
  const NavBtn = ({ item }: { item: NavItem }) => {
    ...
    return (
      <button onClick={()=>{ setTab(item.id); onClose?.(); }} className={cn(
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between p-5 pb-4">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">Personal Finance</p>
        </div>
        {onClose && <Button variant="ghost" size="icon-sm" onClick={onClose}><X className="h-4 w-4"/></Button>}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
        ...
        {ungrouped.map(n=><NavBtn key={n.id} item={n}/>)}
        ...
        {groups.filter(Boolean).map(group=>(
          <div key={group}>
            ...
            {NAV.filter(n=>n.group===group).map(n=><NavBtn key={n.id} item={n}/>)}
            <div className="h-2"/>
          </div>
        ))}
      </nav>
      ...
      <div className="p-4 border-t border-border">
        <div className="flex items-center gap-2">
          <div className={cn("h-2 w-2 rounded-full", state.syncStatus==="firebase"?"bg-profit animate-pulse":"bg-border")}/>
          <p className="text-xs text-muted-foreground">{state.syncStatus==="firebase"?"Firebase live":"Local only"}</p>
        </div>
      </div>
    </div>
  );
...
function AppShell() {
  ...
  if (state.error) return (
    <div className="flex h-dvh items-center justify-center bg-background p-6 text-center">
      <div><p className="text-loss font-bold mb-2">Failed to load</p><p className="text-sm text-muted-foreground">{state.error}</p></div>
    </div>
  );
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-background">
      ...
      {drawer && (
        <>
          <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm md:hidden" onClick={()=>setDrawer(false)}/>
          <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-card border-r border-border md:hidden animate-slide-up">
            <Sidebar tab={tab} setTab={(t)=>{ setTab(t); setDrawer(false); }} onClose={()=>setDrawer(false)}/>
          </aside>
        </>
      )}
      ...
      <div className="flex flex-1 flex-col min-w-0">
        ...
        <header className="flex items-center justify-between border-b border-border bg-card/80 backdrop-blur-md px-4 py-3 md:hidden sticky top-0 z-30">
          <button onClick={()=>setDrawer(true)} className="rounded-lg p-1.5 hover:bg-accent transition-colors">
            <Menu className="h-5 w-5"/>
          </button>
          <div className="flex items-center gap-2">
            {state.syncStatus==="firebase" && <div className="h-1.5 w-1.5 rounded-full bg-profit animate-pulse"/>}
            <span className="font-display font-bold text-base">{activeItem?.label ?? "FinTracker"}</span>
          </div>
          <div className="w-8"/>
        </header>
        ...
        <nav className="fixed bottom-0 left-0 right-0 border-t border-border bg-card/90 backdrop-blur-md md:hidden pb-safe z-30">
          <div className="flex">
            ...
            <button onClick={()=>setDrawer(true)} className="flex flex-1 flex-col items-center gap-1 py-2.5 text-[10px] font-semibold text-muted-foreground">
              <Menu className="h-5 w-5"/>
              More
            </button>
          </div>
