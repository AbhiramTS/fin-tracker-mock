...
interface EntityViewProps<TRecord extends BaseRecord> {
  title: string;
  sub?: string;
  entity: EntityName;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  FormComp: ComponentType<{ initialData?: Partial<TRecord>; onSave: (data: Partial<TRecord>) => void; onCancel: () => void; [k: string]: any }>;
  formProps?: Record<string, unknown>;
  children: ReactNode;
}

export function EntityView<TRecord extends BaseRecord>({
  title, sub, entity, FormComp, formProps = {}, children,
}: EntityViewProps<TRecord>) {
  const { save } = useApp();
