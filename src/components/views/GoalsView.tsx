...
export function GoalsView() {
    ...
    return (
      <Card className={g.status==="completed"?"opacity-70":""}>
        <CardContent className="p-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  ...
                  {g.status==="completed" && <Badge variant="profit">✓ Done</Badge>}
                  {g.status==="paused"    && <Badge variant="secondary">Paused</Badge>}
                  {isOverdue              && <Badge variant="destructive">Overdue</Badge>}
                </div>
              </div>
            </div>
            <Button size="icon-sm" variant="destructive" onClick={()=>remove("goals",g.id)}><Trash2 className="h-3.5 w-3.5"/></Button>
          </div>
          ...
          <Progress value={pct} className="h-2" indicatorClassName={g.status==="completed"?"bg-profit":""}/>
          ...
          <div className="grid grid-cols-2 gap-2 mt-3">
            {g.targetDate && (
              <div className="rounded-lg bg-muted/40 p-2">
                ...
                <p className="text-xs font-semibold mt-0.5">{fmtDateFull(g.targetDate)}</p>
              </div>
            )}
            {g.monthlyContribution && (
              <div className="rounded-lg bg-muted/40 p-2">
                <p className="text-[10px] text-muted-foreground">Monthly</p>
                <p className="text-xs font-mono font-semibold mt-0.5">{fmt(g.monthlyContribution)}</p>
              </div>
            )}
            {completion && (
              <div className="rounded-lg bg-muted/40 p-2">
          ...
          {g.status==="active" && pct>=100 && (
            <Button size="sm" variant="profit" className="mt-3 w-full" onClick={()=>save("goals",{...g,status:"completed"})}>
              ✓ Mark as Completed
            </Button>
          )}
        </CardContent>
  ...
  return (
    <EntityView title="Financial Goals" subtitle={`${active.length} active · ${completed.length} completed`} entity="goals" FormComp={GoalForm}>
      {state.goals.length===0 && (
        <EmptyState icon="🎯" title="No goals yet" description="Set targets for emergency fund, house down payment, vacation, debt payoff…"/>
      )}

      {active.length>0 && <div className="flex flex-col gap-3">{active.map(g=><GoalCard key={g.id} g={g}/>)}</div>}

      {paused.length>0 && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mt-2">Paused</p>
          <div className="flex flex-col gap-3">{paused.map(g=><GoalCard key={g.id} g={g}/>)}</div>
        </>
      )}

      {completed.length>0 && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mt-2">Completed</p>
          <div className="flex flex-col gap-3">{completed.map(g=><GoalCard key={g.id} g={g}/>)}</div>
        </>
      )}
    </EntityView>
  );
