...
function FirebaseSetup({ onConnect }: { onConnect: (cfg: FirebaseConfig) => void }) {
	...
	const connect = () => {
		try {
			...
			setErr('Could not parse config — try the Paste tab and paste the raw object.');
		}
	};
			...
			<Card>
				...
				<CardContent>
					{connected ? (
						...
						<div className="flex flex-col gap-3">
							<p className="text-sm text-muted-foreground">
								Connect Firestore for real-time sync across all your devices. Data
								lives in IndexedDB first — Firebase is the cloud layer only.
							</p>
							<FirebaseSetup
								onConnect={async (cfg) => {
