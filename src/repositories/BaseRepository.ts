...
export class BaseRepository<T extends BaseRecord> {
  ...
  getAll(): Promise<T[]> { return dbGetAll<T>(this.storeName); }

  async save(record: Omit<T, "id" | "createdAt" | "updatedAt"> & Partial<Pick<T, "id" | "createdAt" | "updatedAt">>): Promise<T> {
    ...
    await enqueueChange({ entity: this.storeName, type: isNew ? "create" : "update", payload: saved as unknown as Record<string, unknown> });
    return saved;
  }
